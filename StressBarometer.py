...
class NamedBytesIO(BytesIO):
    """In-memory uploaded file that keeps the original filename."""


def restore_uploaded_file():
    data = st.session_state.get("uploaded_file_bytes")
    name = st.session_state.get("uploaded_file_name")
    if not data or not name:
        return None
    obj = NamedBytesIO(data)
    obj.name = name
    return obj
...
st.markdown(
    ...
    .workflow-active {
        border-color: #16a34a;
        box-shadow: 0 5px 16px rgba(22,163,74,0.12);
    }
...
def main():
    ...
    with st.sidebar:
        ...
        st.caption(
            "All essential settings are now available directly in the central workflow."
        )
    ...
    # Persistent defaults for the central workflow
    if "classification_mode" not in st.session_state:
        st.session_state.classification_mode = "AI model (Gemini / Gemma)"
    ...
    if "included_shorts" not in st.session_state:
        st.session_state.included_shorts = list(CATEGORY_SHORT.values())
    if "n_input" not in st.session_state:
        st.session_state.n_input = 210
    if "is_summary" not in st.session_state:
        st.session_state.is_summary = True
    if "weighting_label" not in st.session_state:
        st.session_state.weighting_label = "Volume (frequency)"
    if "chart_mode" not in st.session_state:
        st.session_state.chart_mode = "Both"
    if "network_nodes" not in st.session_state:
        st.session_state.network_nodes = 25

    step_titles = [
        (1, "🤖", "Classification", "Choose AI or offline classification"),
        (2, "🧭", "Scientific units", "Select the units included in analysis"),
        (3, "📁", "Upload data", "Load TXT, CSV or XLSX"),
        (4, "⚙️", "Analysis settings", "Set sample, weighting and charts"),
        (5, "🚀", "Run analysis", "Start the complete analysis")
    ]

    st.markdown("### 🧭 Start here — 5-step analysis workflow")
    st.caption(
        "Click a step to open it. Complete the steps from left to right; "
        "the system keeps your selections while you move through the workflow."
    )

    nav_cols = st.columns(5, gap="small")
    for col, (num, icon, title, desc) in zip(nav_cols, step_titles):
        with col:
            active = st.session_state.workflow_step == num
            label = f"{icon} {num}. {title}"
            if st.button(
                label,
                key=f"workflow_nav_{num}",
                use_container_width=True,
                type="primary" if active else "secondary"
            ):
                st.session_state.workflow_step = num
                st.rerun()
            st.markdown(
                f"<div class='workflow-card {'workflow-active' if active else ''}'>"
                f"<div class='workflow-number'>{num}</div>"
                f"<div><b>{title}</b><br><span>{desc}</span></div>"
                f"</div>",
                unsafe_allow_html=True
            )

    st.markdown("<div class='workflow-progress'><div class='workflow-progress-fill' style='width:{}%'></div></div>".format(
        st.session_state.workflow_step * 20
    ), unsafe_allow_html=True)
    ...
    if workflow_step == 1:
        with st.container(border=True):
            st.markdown("## 1. 🤖 Choose classification method")
            st.caption(
                "AI classification provides contextual classification with Google Gemini/Gemma. "
                "Dictionary mode works offline and does not require an API key."
            )
            ...
            model_name = st.session_state.get("model_name", AVAILABLE_MODELS[0])
            ...
            if classification_mode.startswith("AI"):
                ...
                model_name = st.selectbox(
                    ...
                    if model_name in AVAILABLE_MODELS else 0,
                    key="model_name"
                )
            ...
            st.success("Classification method selected. Continue to Step 2.")

            c1, c2 = st.columns([1, 1])
            with c2:
                if st.button("Next: Scientific units →", key="next_step_1", use_container_width=True):
                    st.session_state.workflow_step = 2
                    st.rerun()

    else:
        classification_mode = st.session_state.classification_mode
        api_key = st.session_state.get("api_key", "")
        model_name = st.session_state.get("model_name", AVAILABLE_MODELS[0])
        batch_size = st.session_state.get("batch_size", 15)

    # --------------------------------------------------------
    # STEP 2 — SCIENTIFIC UNITS
    # --------------------------------------------------------

    if workflow_step == 2:
        with st.container(border=True):
            st.markdown("## 2. 🧭 Select scientific units")
            st.caption(
                "Choose which scientific units will participate in classification, aggregation and visualization."
            )

            included_shorts = st.multiselect(
                "Included scientific units",
                list(CATEGORY_SHORT.values()),
                default=st.session_state.included_shorts,
                key="included_shorts"
            )
            ...
            if not active_categories:
                ...
                st.warning("No unit was selected; all scientific units will be used.")

            st.info(
                "Active units: " + " · ".join(active_categories),
                icon="🧭"
            )

            c1, c2 = st.columns([1, 1])
            with c1:
                if st.button("← Back to classification", key="back_step_2", use_container_width=True):
                    st.session_state.workflow_step = 1
                    st.rerun()
            with c2:
                if st.button("Next: Upload data →", key="next_step_2", use_container_width=True):
                    st.session_state.workflow_step = 3
                    st.rerun()

    else:
        included_shorts = st.session_state.included_shorts
        active_categories = [SHORT_TO_FULL[s] for s in included_shorts]
        if not active_categories:
            active_categories = list(CATEGORIES_MAP.keys())
    ...
    if workflow_step == 3:
        with st.container(border=True):
            ...
            st.caption(
                "Supported formats: TXT (tab-separated), CSV and XLSX. "
                "After uploading, proceed to Step 4."
            )
            ...
            if uploaded_file is not None:
                ...
                st.info("Choose a data file to continue.", icon="📁")
                st.session_state.analysis_triggered = False
            ...
            c1, c2 = st.columns([1, 1])
            with c1:
                if st.button("← Back to scientific units", key="back_step_3", use_container_width=True):
                    st.session_state.workflow_step = 2
                    st.rerun()
            with c2:
                if st.button(
        ...
        uploaded_file = restore_uploaded_file()
    ...
    if workflow_step == 4:
        with st.container(border=True):
            st.markdown("## 4. ⚙️ Configure the analysis")
            st.caption(
                "Set the sample size, summary status, weighting and visualization preferences."
            )
            ...
            with c1:
                n_input = st.number_input(
                    ...
                    min_value=1,
                    value=int(st.session_state.n_input),
                    key="n_input"
                )
            with c2:
                is_summary = st.checkbox(
                    "The file contains a SUMMARY",
                    value=st.session_state.is_summary,
                    key="is_summary"
                )
            with c3:
            ...
            with c2:
                network_nodes = st.slider(
                    ...
                    min_value=5,
                    max_value=50,
                    value=int(st.session_state.network_nodes),
                    step=1,
                    key="network_nodes",
            ...
            if uploaded_file is None:
                st.warning("Upload a data file in Step 3 before running the analysis.")

            c1, c2 = st.columns([1, 1])
            with c1:
                if st.button("← Back to upload", key="back_step_4", use_container_width=True):
                    st.session_state.workflow_step = 3
                    st.rerun()
            with c2:
                if st.button(
        ...
        n_input = st.session_state.n_input
        is_summary = st.session_state.is_summary
        weighting_label = st.session_state.weighting_label
        weighting_mode = "volume" if "Volume" in weighting_label else "concentration"
    ...
    if workflow_step == 5:
        with st.container(border=True):
            st.markdown("## 5. 🚀 Review and run the analysis")
            st.caption("Everything is ready when the five-step workflow is complete.")

            summary_cols = st.columns(5)
            summaries = [
                ("🤖", "Method", "AI" if st.session_state.classification_mode.startswith("AI") else "Offline"),
                ("🧭", "Units", str(len(st.session_state.included_shorts))),
                ("📁", "File", "Loaded" if st.session_state.get("workflow_file_uploader") else "Missing"),
                ("👥", "N", str(st.session_state.n_input)),
                ("📊", "Charts", st.session_state.chart_mode)
            ]
            for col, (icon, label, value) in zip(summary_cols, summaries):
                with col:
                    st.markdown(
                        f"<div class='workflow-summary'><div class='workflow-summary-icon'>{icon}</div>"
                        f"<div class='workflow-summary-label'>{label}</div><div class='workflow-summary-value'>{html.escape(value)}</div></div>",
                        unsafe_allow_html=True
                    )
            ...
            if uploaded_file is None:
                st.error("Step 3 is incomplete: please upload a data file.")
            elif classification_mode.startswith("AI") and (not api_key or model_name == AVAILABLE_MODELS[0]):
                st.warning("Complete Step 1: enter the Google AI API key and select a model.")
            else:
                st.success(
                    "Workflow complete. Click the button below to start classification, "
                    "stress-degree calculation, kcal/energy analysis and visual reporting."
                )

            run_clicked = st.button(
                "🚀 RUN COMPLETE ANALYSIS",
                key="central_action_btn",
                use_container_width=True,
                type="primary",
                disabled=(uploaded_file is None or (
                    classification_mode.startswith("AI") and
                    (not api_key or model_name == AVAILABLE_MODELS[0])
                ))
            )
            ...
            c1, c2 = st.columns([1, 1])
            with c1:
                if st.button("← Back to analysis settings", key="back_step_5", use_container_width=True):
                    st.session_state.workflow_step = 4
                    st.rerun()
            with c2:
                if uploaded_file is not None and not st.session_state.get("analysis_triggered", False):
                    st.caption("Ready — the analysis has not started yet.")

    # --------------------------------------------------------
    # Keep values available when the workflow is not on step 5
    # --------------------------------------------------------

    uploaded_file = restore_uploaded_file()
    classification_mode = st.session_state.get("classification_mode", "AI model (Gemini / Gemma)")
    api_key = st.session_state.get("api_key", "")
    model_name = st.session_state.get("model_name", AVAILABLE_MODELS[0])
    batch_size = st.session_state.get("batch_size", 15)
    included_shorts = st.session_state.get("included_shorts", list(CATEGORY_SHORT.values()))
    active_categories = [SHORT_TO_FULL[s] for s in included_shorts]
    if not active_categories:
        active_categories = list(CATEGORIES_MAP.keys())
    n_input = st.session_state.get("n_input", 210)
    is_summary = st.session_state.get("is_summary", True)
    weighting_label = st.session_state.get("weighting_label", "Volume (frequency)")
    weighting_mode = "volume" if "Volume" in weighting_label else "concentration"
    chart_mode = st.session_state.get("chart_mode", "Both")
    network_nodes = st.session_state.get("network_nodes", 25)
    ...
    if not uploaded_file:
        ...
        st.info(
            "📁 Start with Step 3 above: upload a TXT, CSV or XLSX file. "
            "The workflow replaces the former infographic and keeps every essential action in the main page.",
            icon="ℹ️"
        )
    ...
    if not st.session_state.get(
        ...
        st.info(
            "▶️ File loaded. Continue to **Step 4** and then **Step 5** in the central workflow to run the analysis.",
            icon="▶️"
        )
